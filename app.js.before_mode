...
function toggleSatellite() {
    if (currentMode === 'plan') { map.removeLayer(planLayer); satelliteLayer.addTo(map); currentMode = 'satellite'; document.getElementById('satBtn').innerHTML = '<i class="fas fa-map"></i>'; }
    else { map.removeLayer(satelliteLayer); planLayer.addTo(map); currentMode = 'plan'; document.getElementById('satBtn').innerHTML = '<i class="fas fa-satellite"></i>'; }
}
...
function toggleTheme() { document.body.classList.toggle('light'); const icon = document.getElementById('themeIcon'); icon.className = document.body.classList.contains('light') ? 'fas fa-sun' : 'fas fa-moon'; localStorage.setItem('tracker_theme', document.body.classList.contains('light') ? 'light' : 'dark'); }
if (localStorage.getItem('tracker_theme') === 'light') { document.body.classList.add('light'); document.getElementById('themeIcon').className = 'fas fa-sun'; }
...
function calcDistance(lat1, lng1, lat2, lng2) { const R = 6371000; const dLat = (lat2 - lat1) * Math.PI / 180; const dLng = (lng2 - lng1) * Math.PI / 180; const a = Math.sin(dLat/2)**2 + Math.cos(lat1*Math.PI/180)*Math.cos(lat2*Math.PI/180)*Math.sin(dLng/2)**2; return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a)); }
...
function calcBearing(lat1, lng1, lat2, lng2) { const dLng = (lng2 - lng1) * Math.PI / 180; const l1 = lat1 * Math.PI / 180, l2 = lat2 * Math.PI / 180; const y = Math.sin(dLng) * Math.cos(l2); const x = Math.cos(l1) * Math.sin(l2) - Math.sin(l1) * Math.cos(l2) * Math.cos(dLng); return (Math.atan2(y, x) * 180 / Math.PI + 360) % 360; }

async function getAddress(lat, lng) {
    const key = lat.toFixed(3) + ',' + lng.toFixed(3);
    if (GEOCODE_CACHE[key]) return GEOCODE_CACHE[key];
    try { const c = new AbortController(); const t = setTimeout(() => c.abort(), 5000); const r = await fetch('https://nominatim.openstreetmap.org/reverse?lat=' + lat + '&lon=' + lng + '&format=json&accept-language=fr', { signal: c.signal, headers: { 'User-Agent': 'TrackerGPS/1.0' } }); clearTimeout(t); const data = await r.json(); const addr = data.display_name ? data.display_name.split(',').slice(0, 2).join(',') : ''; GEOCODE_CACHE[key] = addr; return addr; } catch (e) { return ''; }
}

let audioContext = null;
function playBeep() { try { if (!audioContext) audioContext = new (window.AudioContext || window.webkitAudioContext)(); const osc = audioContext.createOscillator(); const gain = audioContext.createGain(); osc.connect(gain); gain.connect(audioContext.destination); osc.frequency.value = 800; osc.type = 'sine'; gain.gain.setValueAtTime(0.1, audioContext.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + 0.2); osc.start(); osc.stop(audioContext.currentTime + 0.2); } catch (e) {} }

let adminInterval = null, adminLastSent = null, isSendingAdmin = false;
const adminFilter = new KalmanFilter();

function getPosition() { return new Promise((resolve, reject) => { if (!navigator.geolocation) { reject(new Error('Non supportée')); return; } navigator.geolocation.getCurrentPosition(resolve, reject, { enableHighAccuracy: true, timeout: 30000, maximumAge: 0 }); }); }
...
async function sendAdminPosition() {
    ...
    try {
        ...
        if (acc > MAX_ACCURACY) { updateStatusBar('❌ Précision ' + Math.round(acc) + 'm', '#ff5f57'); return; }
        const filtered = adminFilter.process(pos.coords.latitude, pos.coords.longitude, acc);
        if (adminLastSent) { const dist = calcDistance(adminLastSent.lat, adminLastSent.lng, filtered.lat, filtered.lng); if (dist < MIN_MOVE_UPDATE) { const label = getAccuracyLabel(acc); updateStatusBar('📍 Stable ±' + Math.round(acc) + 'm (' + label.text + ')', label.color); return; } }
        adminLastSent = filtered;
        const label = getAccuracyLabel(acc);
        updateStatusBar('📍 ±' + Math.round(acc) + 'm — ' + label.text, label.color);
        await fetch(BACKEND_URL + '/api/position', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, name: 'Admin', whatsapp: '', lat: filtered.lat, lng: filtered.lng, speed: pos.coords.speed || 0, accuracy: acc, heading: pos.coords.heading || 0, altitude: pos.coords.altitude || 0 }) });
...
async function forceRefresh() { const btn = document.getElementById('refreshBtn'); if (btn) btn.style.animation = 'spin 1s linear infinite'; updateStatusBar('🔄 Actualisation...', '#00d4ff'); await sendAdminPosition(); await fetchPositions(); setTimeout(() => { if (markers[userId]) map.setView(markers[userId].marker.getLatLng(), 17, { animate: true }); }, 500); setTimeout(() => { if (btn) btn.style.animation = ''; updateStatusBar('✅ Actualisé', '#28c840'); }, 1500); }

function exportCSV() { const h = histories[userId]; if (!h || h.length < 2) { alert('Pas assez de points'); return; } let csv = 'Index,Latitude,Longitude,Timestamp\n'; h.forEach((pt, i) => { csv += (i + 1) + ',' + pt[0] + ',' + pt[1] + ',' + new Date().toISOString() + '\n'; }); const blob = new Blob([csv], { type: 'text/csv' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'trajectoire_' + Date.now() + '.csv'; a.click(); URL.revokeObjectURL(url); updateStatusBar('📊 CSV (' + h.length + ' pts)', '#28c840'); }

async function captureMap() {
    updateStatusBar('📸 Capture...', '#ffbd2e');
    try {
        ...
        if (buttons) buttons.style.display = 'none'; if (sidebar) sidebar.style.display = 'none'; if (header) header.style.display = 'none'; if (statusBar) statusBar.style.display = 'none';
        ...
        if (buttons) buttons.style.display = origB; if (sidebar) sidebar.style.display = origS; if (header) header.style.display = ''; if (statusBar) statusBar.style.display = '';
        canvas.toBlob(blob => { const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'carte_' + Date.now() + '.png'; a.click(); URL.revokeObjectURL(url); updateStatusBar('📸 Capture OK', '#28c840'); });
    } catch (e) { updateStatusBar('❌ ' + e.message, '#ff5f57'); }
}
...
function createIcon(color, name, bearing, isMe) { const arrow = bearing !== null ? getArrow(bearing) : ''; return L.divIcon({ className: 'custom-marker', html: '<div class="marker-direction">' + arrow + '</div><div class="marker-pin ' + (isMe ? 'me' : '') + '" style="background:' + color + ';"><span>' + name.charAt(0).toUpperCase() + '</span></div>', iconSize: [32, 42], iconAnchor: [16, 32] }); }
...
function callWhatsAppDirect(uid) { const phone = whatsappNumbers[uid]; const marker = markers[uid]; if (!phone) { alert('WhatsApp non dispo'); return; } const name = marker ? marker.name : ''; const message = encodeURIComponent('Bonjour ' + name + ', je te contacte depuis le tracker GPS.'); window.open('https://wa.me/' + phone.replace(/[^0-9]/g, '') + '?text=' + message, '_blank'); updateStatusBar('📞 WhatsApp → ' + name, '#25d366'); }
...
function showRouteTo(targetUserId) {
    ...
    routingControl = L.Routing.control({ waypoints: [L.latLng(mePos.lat, mePos.lng), L.latLng(targetPos.lat, targetPos.lng)], routeWhileDragging: false, addWaypoints: false, fitSelectedRoutes: true, showAlternatives: false, lineOptions: { styles: [{ color: '#ffd700', weight: 4, opacity: 0.8 }] }, createMarker: function() { return null; }, language: 'fr', show: false }).addTo(map);
    routingControl.on('routesfound', function(e) { const r = e.routes[0]; updateStatusBar('🛣️ ' + (r.summary.totalDistance/1000).toFixed(2) + ' km — ' + Math.round(r.summary.totalTime/60) + ' min', '#ffd700'); });
}

let targetMode = false, targetMarker = null;
function toggleTargetMode() { targetMode = !targetMode; const btn = document.getElementById('targetBtn'); if (btn) { btn.style.background = targetMode ? 'rgba(0,212,255,.3)' : ''; btn.style.borderColor = targetMode ? '#00d4ff' : ''; } updateStatusBar(targetMode ? '🎯 Cliquez sur la carte' : '📍 Normal', targetMode ? '#00d4ff' : '#ffbd2e'); document.body.style.cursor = targetMode ? 'crosshair' : ''; }
...
function routeToTarget(lat, lng) { const me = markers[userId]; if (!me) { updateStatusBar('❌ Position non détectée', '#ff5f57'); return; } if (targetRouteControl) map.removeControl(targetRouteControl); const mePos = me.marker.getLatLng(); targetRouteControl = L.Routing.control({ waypoints: [L.latLng(mePos.lat, mePos.lng), L.latLng(lat, lng)], routeWhileDragging: false, addWaypoints: false, fitSelectedRoutes: true, showAlternatives: false, lineOptions: { styles: [{ color: '#00d4ff', weight: 4, opacity: 0.9 }] }, createMarker: function() { return null; }, language: 'fr', show: false }).addTo(map); targetRouteControl.on('routesfound', function(e) { const r = e.routes[0]; updateStatusBar('🛣️ ' + (r.summary.totalDistance/1000).toFixed(2) + ' km — ' + Math.round(r.summary.totalTime/60) + ' min', '#00d4ff'); }); map.closePopup(); }
function clearTarget() { if (targetMarker) { map.removeLayer(targetMarker); targetMarker = null; } if (targetRouteControl) { map.removeControl(targetRouteControl); targetRouteControl = null; } updateStatusBar('🗑️ Effacé', '#ffbd2e'); }
...
function savePlace(id) { const input = document.getElementById('place-name-' + id); if (!input) return; const n = input.value.trim() || 'Lieu'; const p = places.find(x => x.id === id); if (p) { p.name = n; localStorage.setItem(PLACES_KEY, JSON.stringify(places)); updateStatusBar('💾 ' + n, '#28c840'); map.closePopup(); } }
...
function routeToPlace(id) { const place = places.find(p => p.id === id); if (!place) return; const me = markers[userId]; if (!me) { updateStatusBar('❌ Position non détectée', '#ff5f57'); return; } if (placeRouteControl) map.removeControl(placeRouteControl); const mePos = me.marker.getLatLng(); placeRouteControl = L.Routing.control({ waypoints: [L.latLng(mePos.lat, mePos.lng), L.latLng(place.lat, place.lng)], routeWhileDragging: false, addWaypoints: false, fitSelectedRoutes: true, showAlternatives: false, lineOptions: { styles: [{ color: '#ff5f57', weight: 4, opacity: 0.8 }] }, createMarker: function() { return null; }, language: 'fr', show: false }).addTo(map); placeRouteControl.on('routesfound', function(e) { const r = e.routes[0]; updateStatusBar('🛣️ ' + place.name + ': ' + (r.summary.totalDistance/1000).toFixed(2) + ' km — ' + Math.round(r.summary.totalTime/60) + ' min', '#ff5f57'); }); map.closePopup(); }
function togglePlaceMode() { placeMode = !placeMode; const btn = document.getElementById('placeBtn'); if (btn) { btn.style.background = placeMode ? 'rgba(255,95,87,.3)' : ''; btn.style.borderColor = placeMode ? '#ff5f57' : ''; } updateStatusBar(placeMode ? '📌 Cliquez pour épingler' : '📍 Normal', placeMode ? '#ff5f57' : '#ffbd2e'); document.body.style.cursor = placeMode ? 'crosshair' : ''; }
function clearRoute() { if (routingControl) { map.removeControl(routingControl); routingControl = null; } if (placeRouteControl) { map.removeControl(placeRouteControl); placeRouteControl = null; } if (targetRouteControl) { map.removeControl(targetRouteControl); targetRouteControl = null; } updateStatusBar('🗑️ Effacé', '#ffbd2e'); }
...
function togglePlanes() {
    ...
    if (planesEnabled) { if (btn) { btn.style.background = 'rgba(0,212,255,.3)'; btn.style.borderColor = '#00d4ff'; } updateStatusBar('✈️ Chargement...', '#00d4ff'); fetchPlanes(); planesInterval = setInterval(fetchPlanes, 15000); }
    else { if (btn) { btn.style.background = ''; btn.style.borderColor = ''; } if (planesInterval) clearInterval(planesInterval); Object.keys(planeMarkers).forEach(id => { map.removeLayer(planeMarkers[id]); delete planeMarkers[id]; }); updateStatusBar('✈️ Masqués', '#ffbd2e'); }
}
...
function updatePlanesOnMap(planes) {
    ...
    planes.forEach(p => {
        ...
        else { const m = L.marker([lat, lng], { icon }).addTo(map); m.bindPopup('<b>✈️ ' + cs + '</b><br>Alt: ' + Math.round(alt) + ' ft<br>Vit: ' + Math.round(spd) + ' kt'); planeMarkers[id] = m; }
    });
...
function updateMap(positions) {
    ...
    positions.forEach(p => {
        ...
        if (!knownUsers.has(p.user_id)) { knownUsers.add(p.user_id); if (p.user_id !== userId) { playBeep(); updateStatusBar('🔔 ' + p.name, '#28c840'); } }
        ...
        let bearing = null;
        if (histories[p.user_id] && histories[p.user_id].length > 0) { const last = histories[p.user_id][histories[p.user_id].length - 1]; if (last[0] !== smooth.lat || last[1] !== smooth.lng) { bearing = calcBearing(last[0], last[1], smooth.lat, smooth.lng); bearings[p.user_id] = bearing; } }
...
function selectUser(uid) { focusUser(uid); if (uid !== userId) setTimeout(() => { if (confirm('🛣️ Itinéraire ?')) showRouteTo(uid); }, 500); }
